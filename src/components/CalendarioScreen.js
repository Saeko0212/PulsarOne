...
import {
  ...
  TouchableOpacity,
  StyleSheet,
  FlatList, // <--- FlatList ahora será nuestro componente principal
  ActivityIndicator,
  SafeAreaView,
  Alert,
} from 'react-native';
...
const CalendarioScreen = () => {
  ...
  // (Aquí van tus 3 useEffects para cargar 'weeklyEvents', 'monthDots', y 'events')
  
  // useEffect para "Esta Semana"
  useEffect(() => {
    if (!user) {
      setWeeklyEvents([]);
  ...
  // useEffect para eventos del DÍA
  useEffect(() => {
  ...
  // --- Lógica de `markedDates` (sin cambios) ---
  ...
  // --- *NUEVO*: Componente de Cabecera para el FlatList ---
  // Esto agrupa todo lo que va ANTES de la lista de eventos diarios
  const renderListHeader = () => (
    <View>
      ...
      {/* 4. Encabezado del día (movido aquí) */}
  ...
  // --- ESTRUCTURA DEL RETURN (MODIFICADA) ---
  return (
    <SafeAreaView style={styles.container}>
      
      {/* El FlatList de eventos diarios ahora es el componente principal.
        Todo lo demás se carga en su 'ListHeaderComponent'.
      */}
      <FlatList
        data={events}
        renderItem={renderEventItem}
        keyExtractor={(item) => item.id}
        style={styles.mainScrollableList} // <--- Nuevo estilo
        contentContainerStyle={styles.listContent} // <--- Estilo existente
        
        // Carga la cabecera que definimos arriba
        ListHeaderComponent={renderListHeader}
        
        // Maneja el estado de carga y de lista vacía
        ListEmptyComponent={
          loadingDaily ? (
...
const styles = StyleSheet.create({
  container: {
    flex: 1,
    backgroundColor: '#f8f9fa', // Fondo de la safe area
  },
  
  // --- NUEVO Estilo para el FlatList principal ---
  mainScrollableList: {
    flex: 1,
    backgroundColor: 'white',
    borderTopLeftRadius: 20,
    borderTopRightRadius: 20,
    marginTop: -5, // Mantiene el solapamiento
    shadowColor: '#000',
  ...
  headerContainer: {
    ...
    paddingTop: 10,
    paddingBottom: 5,
    backgroundColor: '#f8f9fa', // Fondo del header
  },
  headerTitle: {
    fontSize: 26,
    fontWeight: 'bold',
  ...
  // --- ESTILO 'dayViewContainer' ELIMINADO ---

  // --- Estilo 'dayHeader' MODIFICADO ---
  // (Se le añadió el paddingTop que antes estaba en dayViewContainer)
  dayHeader: {
    flexDirection: 'row',
  ...
  // --- Estilo 'loader' (sin cambios) ---
  loader: {
    marginTop: 50,
  },
  
  // --- Estilo 'listContent' (sin cambios) ---
  // (Ahora aplica al 'contentContainerStyle' del FlatList principal)
  listContent: {
    paddingHorizontal: 20,
    paddingBottom: 20,
  },
  
  // --- Estilos de Lista Vacía y Tarjetas (sin cambios) ---
  emptyContainer: {
    flex: 1,
    justifyContent: 'center',
    alignItems: 'center',
  ...
  eventCard: {
    ...
    padding: 15,
    marginBottom: 15,
    // Las tarjetas individuales ya no necesitan sombra
    // si el contenedor 'mainScrollableList' ya la tiene
  },
  tagContainer: {
    borderRadius: 15,
